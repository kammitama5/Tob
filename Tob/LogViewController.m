...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        ...
        [self->_logTextView setFrame:logTextViewFrame];
        [self->_navbar setFrame:navbarFrame];
        [self->_titleLabel setFrame:CGRectMake(navbarFrame.origin.x, [UIApplication sharedApplication].statusBarFrame.size.height, navbarFrame.size.width, navbarFrame.size.height - [UIApplication sharedApplication].statusBarFrame.size.height)];
        [self->_doneButton setFrame:CGRectMake(10, [UIApplication sharedApplication].statusBarFrame.size.height, 100, navbarFrame.size.height - [UIApplication sharedApplication].statusBarFrame.size.height)];
    } completion:nil];
}
