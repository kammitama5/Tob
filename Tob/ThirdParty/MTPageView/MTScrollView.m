...
- (void)cancelTabActions {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        [self->draggedContainer setCenter:self->originalCenter];
    } completion:nil];
    ...
    [UIView animateWithDuration:kMovingHighlightAnimationDuration animations:^{
        [self->movedContainer.leftArrow setAlpha:0.0];
        [self->movedContainer.rightArrow setAlpha:0.0];
        [self->movedContainer setAlpha:1.0];
        [self->draggedContainer setAlpha:1.0];
    ...
    [UIView animateWithDuration:0.10 animations:^{
        ...
        [[self.parentController tabTitleLabel] setAlpha:self->_parentController.tabsAreVisible];
    }];
            ...
            case UIGestureRecognizerStateEnded: {
                if (-yDistance <= self.frame.size.height / 3 && vel.y >= -1500) {
                    ...
                    [UIView animateWithDuration:0.2 animations:^{
                        self->draggedContainer.alpha = 0;
                        [self->draggedContainer setCenter:CGPointMake(self->originalCenter.x, -self->originalCenter.y)];
                    } completion:^(BOOL finished) {
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gesture {
    ...
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            [UIView animateWithDuration:kMovingHighlightAnimationDuration animations:^{
                [self->draggedContainer setAlpha:kDraggedAlpha];
                CGAffineTransform translation = CGAffineTransformMakeTranslation(self->draggedContainer.transform.tx, self->draggedContainer.transform.ty);
                CGAffineTransform transform = CGAffineTransformScale(translation, self->draggedContainer.transform.a * kMovingSizeFactor, self->draggedContainer.transform.d * kMovingSizeFactor);
                [self->draggedContainer setTransform:transform];
                
                float touchDistance = self->touchLocation.x - self.frame.size.width / 2;
                [self->draggedContainer setCenter:CGPointMake(self->originalCenter.x + touchDistance, self->originalCenter.y)];
            } completion:^(BOOL finished) {
                self->moveScrollTimer = [NSTimer scheduledTimerWithTimeInterval:kScrollMoveDuration target:self selector:@selector(scrollToMoveTab) userInfo:nil repeats:YES];
            }];
...
- (void)scrollToMoveTab {
    ...
    if (movableContainer != movedContainer) {        
        ...
        [UIView animateWithDuration:kScrollMoveDuration delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self->movedContainer = movableContainer;
        }];
    } else {
        [UIView animateWithDuration:kScrollMoveDuration delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
            // Find on which side the dragged container is
    ...
    [UIView animateWithDuration:kScrollMoveDuration delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        float touchDistance = self->touchLocation.x - self.frame.size.width / 2;
        float scrolledDistance = xContentOffset - self->originalContentOffset.x;
        [self->draggedContainer setCenter:CGPointMake(self->originalCenter.x + touchDistance + scrolledDistance, self->originalCenter.y)];
    } completion:^(BOOL finished) {
