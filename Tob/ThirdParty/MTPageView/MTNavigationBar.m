...
- (void)setTouchLocation:(CGPoint)touchLocation {
    ...
    if (movement < -movementToSwitch && selectedShortcutIndex == 1) {
        ...
        [UIView animateWithDuration:kNavBarShortcutSelectionChangeAnimationDuration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            [self->_shortcutSelectorView setFrame:self->_closeButton.frame];
            [self->_closeButton setTintColor:self.backgroundColor];
            [self->_refreshButton setTintColor:self.tintColor];
            [self->_newTabButton setTintColor:self.tintColor];
        } completion:nil];
        ...
        [UIView animateWithDuration:kNavBarShortcutSelectionChangeAnimationDuration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            [self->_shortcutSelectorView setFrame:self->_newTabButton.frame];
            [self->_newTabButton setTintColor:self.backgroundColor];
            [self->_closeButton setTintColor:self.tintColor];
            [self->_refreshButton setTintColor:self.tintColor];
        } completion:nil];
        ...
        [UIView animateWithDuration:kNavBarShortcutSelectionChangeAnimationDuration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            [self->_shortcutSelectorView setFrame:self->_refreshButton.frame];
            [self->_refreshButton setTintColor:self.backgroundColor];
            [self->_closeButton setTintColor:self.tintColor];
            [self->_newTabButton setTintColor:self.tintColor];
        } completion:nil];
    }
